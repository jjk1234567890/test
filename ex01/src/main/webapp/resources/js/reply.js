...
$(document).ready(function(){
	...
	$(".replyList").on("click","li",function(){
		...
		get(rno);
		// 모달창 안에 있는 rno입력란에 rno를 넣어라
		$("#rno").val(rno);
		// 모달창 안에 있는 댓글쓰기버튼은 숨겨라
		$("#modalWriteBtn").hide();
	...
	$("#modalModBtn").on("click",function(){
		...
		// 댓글내용(reply)값 가져오기
		//$("#reply").val();
		// 모달창안에 있는 댓글 수정 버튼을 클릭하면, 댓글내용이 수정되는 update 기능을 하는 함수 호출
		//modify({댓글번호변수:댓글번호값,댓글내용변수:댓글내용값,게시물번호변수:게시물번호값});
...
function add(reply){
	console.log("댓글 추가.......");
	console.log(reply);
	// ajax()를 이용해서 DB에 Insert하는 것을 작성.
	$.ajax({	// 비동기식으로 통신.
		type:"post",
		url:"/replies/new",
...
//댓글목록리스트를 클릭한 후 띄워지는 모달창에 select된 결과 뿌리기 기능을 하는 함수 선언
function get(rno){
	...
	$.getJSON("/replies/detail/"+rno+".json",function(data){
		console.log(data);
		// 모달창안에 있는 reply를 선택한 후 값을 저장해라. 
		$("#reply").val(data.reply);
		// 모달창안에 있는 replyer를 선택한 후 값을 저장해라.
		$("#replyer").val(data.replyer);
		// 모달창안에 있는 replydate를 선택한 후 숨겨라
		$("#replydateDiv").hide();
	});
}
// 모달창안에 있는 댓글 수정 버튼을 클릭하면, 댓글내용이 수정되는 update 기능을 하는 함수 선언
function modify(reply){
	console.log("댓글 내용 수정.......");
	console.log(reply);
	// ajax()
	$.ajax({
		type:"put",
