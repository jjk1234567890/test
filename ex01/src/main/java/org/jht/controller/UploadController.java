...
public class UploadController {
	...
	private String getFolder() {
		...
		// 현재날짜(오늘날짜) : Sun Dec 17 1995 03:24:00 GMT...
		Date date = new Date();
	...
	public ResponseEntity<ArrayList<AttachFileDTO>> uploadAjaxPost(MultipartFile[] uploadfile) {
		...
		for(MultipartFile multipartFile:uploadfile) {
			...
			System.out.println("사용자가 업로드한 실제 파일명 ajax="+multipartFile.getOriginalFilename());
			System.out.println("사용자가 업로드한 실제 용량크기 ajax="+multipartFile.getSize());
			//File saveFile=new File(파일업로드경로,파일명)
			//File saveFile=new File(uploadFolder,multipartFile.getOriginalFilename());
			...
			// uuid문자열을 AttachFileDTI에 uuid변수에 저장
			...
			try {
				...
				if(checkImageType(saveFile)) {// saveFile에 있는 파일이 checkImageType메소드에 의해서 true면,
					...
					// 섬네일 생성.					 (원본 이미지 복사,     새로운 이미지 생성(UUID_s_파일명), 넓이, 높이)
